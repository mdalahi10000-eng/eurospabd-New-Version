...
export function BookNowModal({
  ...
  const allServices = availableServices && availableServices.length > 0 ? availableServices : SERVICES_DATA;
  const initialService = preselectedService || allServices[0];
  ...
  useEffect(() => {
    if (preselectedService) {
      ...
      if (preselectedOption) {
        setSelectedDuration(preselectedOption.duration);
      } else if (preselectedService.priceOptions?.[0]) {
        setSelectedDuration(preselectedService.priceOptions[0].duration);
      }
    }
