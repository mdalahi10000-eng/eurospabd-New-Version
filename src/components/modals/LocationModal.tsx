...
export function LocationModal({ isOpen, onClose, onCallClick, businessInfo }: LocationModalProps) {
  const [copied, setCopied] = useState(false);

  if (!isOpen) return null;

  const name = businessInfo?.businessName || SPA_INFO.name;
  const fullAddress = businessInfo?.fullAddress || SPA_INFO.fullAddress;
  const googleMapsUrl = businessInfo?.googleMapsUrl || SPA_INFO.googleMapsUrl;
  const locationShort = businessInfo ? `${businessInfo.area}, ${businessInfo.city}` : SPA_INFO.locationShort;
  const hoursDisplay = businessInfo?.displayStatus || `${SPA_INFO.openingHours} Daily`;
        ...
        >
          ...
          <div className="relative h-56 sm:h-64 w-full bg-neutral-100 overflow-hidden border-b border-neutral-200/80">
            ...
            <div className="absolute top-3 left-3 bg-white/95 backdrop-blur-xs px-3 py-2 rounded-xl shadow-md border border-neutral-200 pointer-events-none flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-neutral-900">{name}</span>
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 gap-2.5 text-xs">
              <div className="p-3 bg-white rounded-xl border border-gray-200/80 flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <span className="font-bold text-gray-800">{hoursDisplay}</span>
                </div>
              </div>
