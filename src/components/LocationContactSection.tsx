...
export function LocationContactSection({
  ...
  const locationLabel = businessInfo ? `${businessInfo.addressLine}, ${businessInfo.area}, ${businessInfo.city}` : SPA_INFO.locationShort;
  const phone = businessInfo?.phone || SPA_INFO.phone;
  const whatsappFormatted = businessInfo?.whatsappFormatted || SPA_INFO.whatsappFormatted;
  const email = businessInfo?.email || SPA_INFO.email;
  const status = businessInfo?.displayStatus || SPA_INFO.status;
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-4 shadow-xs flex flex-col justify-between h-[calc(100%-2.25rem)]">
            <div className="space-y-3 pt-0.5">
              ...
              >
                ...
                <span className="text-xs sm:text-sm font-medium text-green-700 font-mono">
                  {whatsappFormatted}
                </span>
              </button>
